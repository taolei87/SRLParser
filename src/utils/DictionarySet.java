...
public class DictionarySet implements Serializable {
    ...
    public void reOrderArgLabel() {
        Dictionary old = dicts[DictionaryTypes.AUGLABEL.ordinal()];
        Dictionary newDict = new Dictionary();

        Object[] keys = old.toArray();
        int id = newDict.lookupIndex(unseen);   // id=1 means unseen item (pos,word,etc.)
        Utils.Assert(id == 1);
        ...
        for (Object key : keys) {
                if (((String)key).indexOf("-") == -1) {
                        maxID = newDict.lookupIndex((String)key);
                }
        }
        ARGINDEX = maxID + 1;
        ...
        for (Object key : keys) {
                if (((String)key).indexOf("-") != -1) {
                        newDict.lookupIndex((String)key);
                }
        }

        dicts[DictionaryTypes.AUGLABEL.ordinal()] = newDict;

        System.out.println("arg label index reordered");
}
}
